  ...
  return (
    <>
      <Helmet>
        <title>
          {lang === 'sl'
            ? 'Oprema – Megama NDT | Ultrazvočna, magnetna in radiografska oprema'
            : 'Equipment – Megama NDT | Ultrasonic, Magnetic & Radiographic Equipment'}
        </title>
        <meta
          name="description"
          content={
            lang === 'sl'
              ? 'Pregled NDT opreme podjetja Megama: ultrazvočni defektoskopi, magnetni prašni sistemi, penetrantski komplet in radiografska oprema.'
              : 'Overview of Megama NDT equipment: ultrasonic flaw detectors, magnetic particle systems, penetrant testing kits and radiographic equipment.'
          }
        />
      </Helmet>
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(0,113,227,0.05)_0%,transparent_100%)]" />
      ...
      <section className="relative pt-40 pb-20 z-20">
        <div className="max-w-[1200px] mx-auto px-6 relative text-center">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-flex items-center gap-3 px-4 py-2 border border-[#0071e3]/30 rounded-full bg-[#0071e3]/5 backdrop-blur-md mb-8">
               <Cpu className="w-4 h-4 text-[#0071e3]" />
               <span className="text-[10px] font-mono tracking-widest text-[#0071e3] font-bold uppercase">{t.equipmentPage.hero_badge}</span>
            </div>
      ...
      <section className="relative z-20 pb-32">
        <div className="max-w-[1200px] mx-auto px-6 grid lg:grid-cols-2 gap-8">
          ...
          <motion.div 
            ...
            className="group bg-[#0a0a0a] border border-white/10 hover:border-[#0071e3]/50 rounded-[40px] overflow-hidden transition-all duration-500 relative flex flex-col"
          >
            {/* Visual Header */}
            <div className="h-[250px] md:h-[300px] bg-black border-b border-white/10 relative overflow-hidden flex items-center justify-center p-10">
               ...
               <motion.div 
                 className="absolute inset-0 opacity-20 border-t border-[#0071e3]"
                 animate={{ y: ['0%', '100%', '0%'] }}
                 transition={{ duration: 4, ease: "linear", repeat: Infinity }}
               />
               <Activity className="w-32 h-32 text-white/5" />
               <div className="absolute inset-0 flex items-center justify-center">
                 <div className="w-64 h-64 border border-[#0071e3]/20 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-700">
                    <div className="w-48 h-48 border border-[#0071e3]/40 rounded-full flex items-center justify-center border-dashed animate-spin-slow">
                       <Target className="w-16 h-16 text-[#0071e3]" />
                    </div>
                 </div>
               </div>
               
               <div className="absolute top-4 left-4 font-mono text-[9px] text-[#0071e3] tracking-widest">
                 SEQ_SCAN: ACTIVE
               </div>
            </div>
          ...
          >
            ...
            <div className="h-[250px] md:h-[300px] bg-black border-b border-white/10 relative overflow-hidden flex items-center justify-center p-10">
               <motion.div 
                 className="absolute w-full h-full opacity-10 bg-[radial-gradient(circle_at_center,rgba(0,113,227,1)_0%,transparent_60%)]"
                 animate={{ scale: [1, 1.2, 1] }}
                 transition={{ duration: 3, repeat: Infinity }}
               />
               <div className="absolute inset-0 flex items-center justify-center gap-10">
                 <motion.div animate={{ x: [-10, 10, -10] }} transition={{ duration: 2, repeat: Infinity }} className="h-40 w-1 bg-[#0071e3]/50" />
                 <Maximize className="w-20 h-20 text-[#0071e3]" />
                 <motion.div animate={{ x: [10, -10, 10] }} transition={{ duration: 2, repeat: Infinity }} className="h-40 w-1 bg-[#0071e3]/50" />
               </div>

               <div className="absolute top-4 right-4 font-mono text-[9px] text-[#0071e3] tracking-widest text-right">
